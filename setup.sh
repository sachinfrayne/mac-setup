...
# brew_install() / brew_install_cask() functions now provided by lib/common.sh
# Package lists (the single source of truth) now provided by lib/packages.sh

for pkg in "${BREW_FORMULAE[@]}"; do
	brew_install "$(pkg_name "$pkg")"
done

for pkg in "${BREW_CASKS[@]}"; do
	brew_install_cask "$(pkg_name "$pkg")"
done
